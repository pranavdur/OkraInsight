...
def load_image(filename):
    ...
    if ext == '.npy':
        ...
        with open(filename, 'rb') as file:
            return pickle.load(file).astype(bool)
    else:
...
class BasicDataset(Dataset):
    ...
    @staticmethod
    def crop_dim1(mask_values, pil_img, scale, is_mask):
        # w, h = pil_img.size
        ...
        if is_mask:
          ...
          area = (0, 750, 4000, 2250)
        cropped_img = pil_img.crop(area)
        newW, newH = cropped_img.size

        scaled_newW, scaled_newH = int(scale * newW), int(scale * newH)
        assert scaled_newW > 0 and scaled_newH > 0, 'Scale is too small, resized images would have no pixel'
        cropped_img = cropped_img.resize((scaled_newW, scaled_newH), resample=Image.NEAREST if is_mask else Image.BICUBIC)
    ...
    def __getitem__(self, idx):
        ...
        assert img.size == mask_as_image.size, \
